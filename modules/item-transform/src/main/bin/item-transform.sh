...
hadoop fs -mkdir -p ${META_DIR}/item

hadoop fs -copyFromLocal -f ${MODULE_HOME}/etc/*.avsc  ${META_DIR}/item/

fn_run_spark_transform_on_yarn transform.XmlToAvro \
    ${MODULE_HOME}/lib/item-transform.jar \
    ${MODULE_HOME}/lib/xml-avro-1.0.0-SNAPSHOT.jar \
    ${MODULE_HOME}/etc/item.xsd \
    ${DB_RAW_DIR}/item/batch_id=${BATCH_ID} \
    ${DB_TRANSFER_DIR}/item/batch_id=${BATCH_ID} \
    item.xsd

fn_run_hive "${MODULE_HOME}" \
    "${MODULE_HOME}/etc/hive/item-transform.hive.properties" \
    "${MODULE_HOME}/etc/schema/item-transform.hql"
